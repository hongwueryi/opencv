...
int opencv2h264()
{
     ...
     */
    double totalFrameCount = videoCapturer.get(CAP_PROP_FRAME_COUNT);
    uint width = videoCapturer.get(CAP_PROP_FRAME_WIDTH),
     ...
     */
    //avcodec_register_all(); //4.0版本之后已经废弃
    const AVCodec* h264encoder = avcodec_find_encoder(destCodec);
